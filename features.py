...
def mfcc(signal, samplerate=16000, winlen=0.025, winstep=0.01, numcep=13,
    ...
    feat = numpy.log(feat)
    feat = dct(feat, type=2, axis=1, norm='ortho')[:, :int(numcep)]
...
def ssc(signal, samplerate=16000, winlen=0.025, winstep=0.01,
    ...
    fb = get_filterbanks(nfilt, nfft, samplerate)
    feat = numpy.dot(pspec, fb.T)  # compute the filterbank energies
    R = numpy.tile(numpy.linspace(1, samplerate/2,
                                  numpy.size(pspec, 1)), (numpy.size(pspec, 0), 1))
...
def get_filterbanks(nfilt=20.0, nfft=512.0, samplerate=16000.0, lowfreq=0, highfreq=None):
